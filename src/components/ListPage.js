...
function ListPage() {
  ...
  const categoryFilterdList = (list) => {
    ...
    if (params.categoryid) {
      _newList = list.filter(item => item.category.find(_item => _item === params.categoryid));
    } else if (params.date) {
  ...
  const renderList = (list) => {
    ...
    const _newList = filterList(list).filter((item, i) => (i >= itemRangeMin && i <= itemRangeMax));
    return _newList;
  }
  const handleNavigate = (pageid) => () => {
    const url = (params.categoryid ? '/category/' + params.categoryid : '') + (params.date ? '/date/' + params.date : '') + '/page/' + pageid + '/';
    navigate(url);
  }
            ...
            <Link to={`/post/${item.id}`}>
              <div className={styles.listItem__wrap}>
                <div className={styles.listItem__imgarea}>
                  <img src={item.thumnail.url} alt="" width={item.thumnail.width} height={item.thumnail.height}></img>
                </div>
                <div className={styles.listItem__txtarea}>
                  <div className={styles.listItem__txtarea__head}>
                    <ul className={styles.category}>
                      {item.category.map((elm, j) => (
                        <li className={styles.categoryItem} key={j}>{elm}</li>
                      ))}
                    </ul>
